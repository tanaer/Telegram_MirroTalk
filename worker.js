...
async function onMessage(message) {
  ...
  // --- 管理员逻辑 ---
  // 如果不是回复消息，则发送帮助信息
  if (!message.reply_to_message || !message.reply_to_message.chat) {
    return sendMessage({
      chat_id: ADMIN_UID,
      text: '使用方法：回复转发的消息。\n指令：\n/trust - 加白名单\n/untrust - 移除白名单\n/block - 屏蔽\n/unblock - 解除屏蔽'
    });
  ...
  switch (message.text) {
    case '/block':
      return updateBlockStatus(replyMsgId, true);
    case '/unblock':
      return updateBlockStatus(replyMsgId, false);
    case '/checkblock':
      return checkBlockStatus(replyMsgId);
    case '/trust':
      return updateTrustStatus(replyMsgId, true);
    case '/untrust':
      return updateTrustStatus(replyMsgId, false);
  }

  // 管理员正常回复
  const guestChatId = await MirroTalk.get('msg-map-' + replyMsgId, { type: "json" });
  if (guestChatId) {
    return copyMessage({
      chat_id: guestChatId,
...
async function handleMediaMessage(message, chatId) {
  // 获取验证状态
  const isVerified = await MirroTalk.get('verified-' + chatId, { type: "json" });
...
// 通用函数：更新屏蔽状态
async function updateBlockStatus(replyMsgId, isBlocking) {
  const guestChatId = await MirroTalk.get('msg-map-' + replyMsgId, { type: "json" });
  ...
  if (isBlocking) {
    ...
    await MirroTalk.put('isblocked-' + guestChatId, true);
    return sendMessage({ chat_id: ADMIN_UID, text: `UID:${guestChatId} 屏蔽成功 (Shadowban)` });
  } else {
    await MirroTalk.delete('isblocked-' + guestChatId);
    return sendMessage({ chat_id: ADMIN_UID, text: `UID:${guestChatId} 解除屏蔽成功` });
  }
}

// 通用函数：检查屏蔽状态
async function checkBlockStatus(replyMsgId) {
  const guestChatId = await MirroTalk.get('msg-map-' + replyMsgId, { type: "json" });
  if (!guestChatId) return;

  const blocked = await MirroTalk.get('isblocked-' + guestChatId, { type: "json" });
  return sendMessage({
    chat_id: ADMIN_UID,
...
// 通用函数：更新信任状态
async function updateTrustStatus(replyMsgId, isTrusting) {
  const guestChatId = await MirroTalk.get('msg-map-' + replyMsgId, { type: "json" });
  if (!guestChatId) return;

  if (isTrusting) {
    await MirroTalk.put('verified-' + guestChatId, true);
    return sendMessage({ chat_id: ADMIN_UID, text: `✅ UID:${guestChatId} 已设为信任。` });
  } else {
    await MirroTalk.delete('verified-' + guestChatId);
    return sendMessage({ chat_id: ADMIN_UID, text: `❎ UID:${guestChatId} 已移除信任。` });
  }
}
